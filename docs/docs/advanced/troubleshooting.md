...
```mermaid
flowchart TD
    Start["❓ 트레이스가 보이지 않음"] --> Q1

    Q1{"SDK가 초기화되었는가?"} -->|확인| A1["로그에서 OTel 초기화 확인"]
    A1 --> Q2

    Q2{"Exporter가 연결되었는가?"} -->|확인| A2["네트워크 연결 테스트"]
    A2 --> Q3

    Q3{"Collector가 수신 중인가?"} -->|확인| A3["Collector 메트릭/로그 확인"]
    A3 --> Q4

    Q4{"샘플링되고 있는가?"} -->|확인| A4["샘플러 설정 확인"]
    A4 --> Q5

    Q5{"백엔드가 정상인가?"} -->|확인| A5["백엔드 상태 확인"]
    A5 --> Done["✅ 문제 해결"]
